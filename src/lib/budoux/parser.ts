...
/*
 * 取得元: google/budoux v0.9.2（タグ v0.9.2 = コミット e00fd7f9e2b5b39b6243fbab072d76c6a6e1f41b）の
 *         javascript/src/parser.ts（npm budoux@0.9.2 の src/parser.ts と同一内容であることを確認済み）
 *
 * FREENOUGHによる変更:
 * - このファイルを src/lib/budoux/parser.ts として配置した（BudouX公式のParser＋モデルの構成を参考にした最小構成）。
 * - Parserクラスのコード（constructor・parse・parseBoundaries）は無変更。スコア計算・しきい値など、
 *   区切り方の結果に関わる処理は一切変えていない。
 * - 公式パッケージのうち、HTMLProcessor（html_processor.ts）・DOM操作（dom.ts・dom-browser.ts）・
 *   Webコンポーネント・CLI・linkedomに関わる部分は取り込んでいない（このファイルはもともとそれらに依存しない）。
 */
