...
export interface FireAgeFormValues {
  curAge: number;
...
interface FireAgeFormProps {
  values: FireAgeFormValues;
  onChange: (patch: Partial<FireAgeFormValues>) => void;
}
...
function NumberField({ label, id, value, suffix, step = 1, help, onChange }: NumberFieldProps) {
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      {help && <p className="text-[11px] text-slate-400 leading-relaxed">{help}</p>}
    </div>
  );
}

export default function FireAgeForm({ values, onChange }: FireAgeFormProps) {
  return (
