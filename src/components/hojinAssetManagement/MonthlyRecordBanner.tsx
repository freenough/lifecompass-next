...
import { isCurrentMonthRecorded } from '@/lib/hojinAssetManagement/monthlyCheck';
...
// 個人資産管理ツールのMonthlyRecordBanner.tsx（ロック対象）を複製し、文言のみ「法人資産」に調整。
export default function MonthlyRecordBanner({ snapshots, onRecord }: MonthlyRecordBannerProps) {
  ...
  return (
    <div className="rounded-lg bg-amber-50 border border-amber-200 px-4 py-3 flex items-center justify-between gap-3">
      <p className="text-xs text-amber-800">今月はまだ記録していません。法人資産の推移を残しておきましょう。</p>
      <button
        onClick={onRecord}
