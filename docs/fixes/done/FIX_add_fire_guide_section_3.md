...
<section>
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
    {featuredPosts.map(post => (
      <a key={post.slug} href={`/blog/${post.slug}`} className="fireguide-card">
        <div className="fireguide-thumb">
          {/* eyecatch画像。190px×約127px(3:2)固定。object-fit: cover */}
          <img src={post.eyecatch} alt="" />
        </div>
        <div className="fireguide-content">
          <h3>{post.title}</h3>
          <p className="excerpt">{post.excerpt}</p>
          {/* 「◯分で読む→」のCTA行は表示しない(note側のタイプ診断カードとの
              統一感を優先。カード全体がリンクなので明示的CTAは不要と判断) */}
        </div>
      </a>
    ))}

    {/* プレースホルダー(4枚目・記事未公開) */}
    <div className="fireguide-card cursor-default">
      <span className="badge-gray">近日公開</span>
      <div className="fireguide-thumb" style={{ background: 'var(--bd2)' }} />
      <div className="fireguide-content">
        <h3>FIRE達成に必要な資産額の計算方法</h3>
        <p className="excerpt">(準備中)</p>
      </div>
    </div>
  </div>
...
- **サムネイル仕様(確定):** 既存3枚のeyecatch画像は全て1536×1024px(3:2比率)で統一されている(確認済み)。この比率をそのままサムネ枠に適用することで、`object-fit: cover`でも意味のある部分が切り抜かれる事故を避ける。
  ...
  /* サムネイル:固定サイズ。カードの高さに追従させない(stretchにすると
     タイトルが3行になった時にサムネ幅も伸びてテキストエリアを圧迫し、
     さらに折り返しが増えて高さが伸びる…という悪循環が起きるため) */
  .fireguide-thumb {
    width: 190px;
    aspect-ratio: 3 / 2;   /* 高さは約127px相当で固定。元画像と同じ比率 */
    flex-shrink: 0;
    align-self: center;    /* カードが縦に伸びても上下の余白として吸収される */
    overflow: hidden;
  }
  .fireguide-thumb img {
    width: 100%;
  ...
  .fireguide-content h3 {
    display: -webkit-box;
    -webkit-line-clamp: 3;   /* 2 → 3 に変更。タイトルの切れを解消 */
  ...
  - カードはアイコン+テキストの縦積みではなく、**サムネイル(左・固定サイズ)+テキスト(右・paddingあり)の横並び**レイアウトにする
  - **「3分で読む→」のCTA行は削除する**(noteのタイプ診断カードにも同様の明示的CTA表現がないため、統一感を優先。カード全体がリンクなのでクリック可能性は視覚的に十分伝わる。`readingTime`のfrontmatterフィールド自体は残してよい。将来別の用途で使う可能性があるため)
